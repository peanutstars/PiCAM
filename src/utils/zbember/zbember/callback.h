 ...
 * release.  Customers should use ::emberAfMainTickCallback instead.
 *
 */
...
/** @brief Cli Internet Server Send
 *
 * This function is called by the framework when it wants to send a CLI
 * constructed ZCL packet over IP.
 *
 * @param apsFrame The structure containing the APS frame  Ver.: always
 * @param destinationEUI Destintion EUI64  Ver.: always
 * @param message Pointer to the message payload  Ver.: always
 * @param messageLength Length of the message payload  Ver.: always
 */
boolean emberAfCliInternetServerSendCallback(EmberApsFrame* apsFrame,
...
/** @name Find and Bind Plugin Callbacks */
// @{

/** @brief Get Target Endpoints
 *
 * This callback lets the user define which endpoints on the target that they
 * want to start identify in the finding and binding process. If this function
 * does nothing, then no endpoints will identify in the finding and binding
 * process.
 *
 * @param endpoints An array of endpoints the size of ::FIXED_ENDPOINT_COUNT 
 * Ver.: always
 */
void emberAfPluginFindAndBindGetTargetEndpointsCallback(int8u *endpoints);

/** @brief Get Initiator Endpoint
 *
 * This callback will enable the user to return the endpoint from which they
 * want to start the initiator binding process. If the user does not wish to
 * initiator from any endpoints, they can return EMBER_ZDP_INAVLID_ENDPOINT.
 *
 */
int8u emberAfPluginFindAndBindGetInitiatorEndpointCallback(void);

/** @brief Found Bind Target
 *
 * This callback with enable the user to programmatically decide if they want to
 * bind with a potential target. The plugin will try to bind with this target if
 * and only if the function returns true. By default, the callback tells the
 * plugin to try to bind with the target.
 *
 * @param nodeId short ID of the potential target  Ver.: always
 * @param endpoint destination endpoint of the potential target  Ver.: always
 * @param ieeeAddr EUI64 of the potential target  Ver.: always
 * @param clusterId cluster of the potential target  Ver.: always
 */
...
/** @} END Find and Bind Plugin Callbacks */
...
/** @brief Unused Pan Id Found
 *
 * This function is called when the form-and-join library finds an unused PAN ID
 * that can be used to form a new network on.
 *
 * @param panId A randomly generated PAN ID without other devices on it.  Ver.:
 * always
 ...
 * function implementor may add additional file descriptors that the application
 * will monitor with select() for data ready.  These file descriptors must be
 * read file descriptors.  The number of file descriptors added must be returned
 * by the function (0 for none added).
 *
 ...
 */
void emberAfPluginSleepyMessageQueueMessageTimedOutCallback(int8u sleepyMsgId);
