...
static void printIncomingZclMessage(const EmberAfClusterCommand *cmd)
{
  ...
  if (emberAfPrintReceivedMessages) {
    ...
    emberAfAppPrint("nodeId[0x%04x] ", cmd->source) ;
...
{
  int8u minLength = (message[0] & ZCL_MANUFACTURER_SPECIFIC_MASK
                                     ...
                                     EmberApsFrame *apsFrame)
{
  ...
  int8u max = EMBER_AF_MAXIMUM_APS_PAYLOAD_LENGTH;
...
int8s emberAfCompareValues(int8u* val1,
                           int8u* val2,
                           int8u len,
                           boolean signedNumber) 
{
  ...
  if (signedNumber){ // signed number comparison 
    if (len <= 4){ // only number with 32-bits or less is supported
      ...
      if (len < 4){
        if (accum1 & (1<<(8*len - 1))){ // check sign
          accum1 |= all1s - ((1 << (len*8)) - 1);
        }
        if (accum2 & (1<<(8*len - 1))){ // check sign
...
boolean emberAfIsTypeSigned(EmberAfAttributeType dataType) {
...
boolean emberAfEndpointEventControlGetActive(EmberEventControl *controls,
                                             int8u endpoint)
{
  int8u index = emberAfIndexFromEndpoint(endpoint);
...
int8u emberAfAppendCharacters(int8u * zclString,
